...
// ==========================================================
// 1. AES-128 S-BOX & MATH COMPONENTS
// ==========================================================

module aes_sbox(input [7:0] in, output reg [7:0] out);
    always @(*) begin
        case (in)
...
module mix_columns(input [31:0] col_in, output [31:0] col_out);
    function [7:0] g2(input [7:0] x); g2 = {x[6:0], 1'b0} ^ (x[7] ? 8'h1b : 8'h00); endfunction
    wire [7:0] b0 = col_in[31:24], b1 = col_in[23:16], b2 = col_in[15:8], b3 = col_in[7:0];
    
    assign col_out[31:24] = g2(b0) ^ (g2(b1) ^ b1) ^ b2 ^ b3;
    assign col_out[23:16] = b0 ^ g2(b1) ^ (g2(b2) ^ b2) ^ b3;
    assign col_out[15:8]  = b0 ^ b1 ^ g2(b2) ^ (g2(b3) ^ b3);
    assign col_out[7:0]   = (g2(b0) ^ b0) ^ b1 ^ b2 ^ g2(b3);
endmodule

// ==========================================================
// 2. AES KEY EXPANSION (DYNAMIC SCHEDULE)
// ==========================================================
...
module aes_key_expansion(
    ...
    // SubWord(RotWord(w3))
    aes_sbox s0(.in(w3[23:16]), .out(sb0));
    aes_sbox s1(.in(w3[15:8]),  .out(sb1));
    aes_sbox s2(.in(w3[7:0]),   .out(sb2));
    aes_sbox s3(.in(w3[31:24]), .out(sb3));

    wire [31:0] g_w3 = {sb0 ^ rcon, sb1, sb2, sb3};

    always @(*) begin
        if (round == 0) begin
            round_key = key_in;
        end else begin
            round_key[127:96] = key_in[127:96] ^ g_w3;
            round_key[95:64]  = key_in[95:64]  ^ round_key[127:96];
            round_key[63:32]  = key_in[63:32]  ^ round_key[95:64];
            round_key[31:0]   = key_in[31:0]   ^ round_key[63:32];
        end
    end
endmodule

// ==========================================================
// 3. AES COPROCESSOR PIPELINE
// ==========================================================

module aes_coprocessor(
    input clk, rst, start,
    input [127:0] plaintext,
    input [127:0] base_key,
    ...
    generate
        for (i=0; i<16; i=i+1) begin : sbox_gen
            aes_sbox sb (.in(state[127-i*8 -: 8]), .out(sb_out[127-i*8 -: 8]));
        end
    endgenerate

    // 3. ShiftRows
    wire [127:0] sr_out;
    assign sr_out = { sb_out[127:120], sb_out[87:80], sb_out[47:40], sb_out[7:0],
                      sb_out[95:88],  sb_out[55:48], sb_out[15:8],  sb_out[103:96],
                      sb_out[63:56],  sb_out[23:16], sb_out[111:104],sb_out[71:64],
                      sb_out[31:24],  sb_out[119:112],sb_out[79:72], sb_out[39:32] };
    
    // 4. MixColumns
    wire [127:0] mc_out;
    mix_columns mc0(sr_out[127:96], mc_out[127:96]);
    mix_columns mc1(sr_out[95:64],  mc_out[95:64]);
    mix_columns mc2(sr_out[63:32],  mc_out[63:32]);
    mix_columns mc3(sr_out[31:0],   mc_out[31:0]);

    // 5. State Machine (Syncing everything to the clock to stop 'X')
// 5. State Machine
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            round <= 0;
            busy <= 0;
            ciphertext <= 128'b0; // Start at 0
            state <= 128'b0;
            current_key <= 128'b0;
        end 
        else if (start && !busy) begin
            state <= plaintext ^ base_key;
            current_key <= base_key;
            round <= 1;
            busy <= 1;
            ciphertext <= 128'b0; // Keep it clear while working
        end 
        else if (busy) begin
            if (round < 10) begin
                state <= mc_out ^ round_key_out;
                current_key <= round_key_out;
                round <= round + 1;
            end 
            else begin
                // ONLY UPDATE THE OUTPUT REGISTER HERE
                ciphertext <= sr_out ^ round_key_out;
                busy <= 0;
                round <= 0;
            end
        end
        // If not busy and not starting, ciphertext holds its value
    end
endmodule

// ==========================================================
// 4. RISC-V CORE & INTEGRATION
// ==========================================================

module program_counter(input clk, rst, stall, input [31:0] pc_in, output reg [31:0] pc_out);
    always @(posedge clk or posedge rst) begin
        if (rst) pc_out <= 32'b0;
...
module Instruction_Memory(input [31:0] addr, output [31:0] instr);
    reg [31:0] Rom [0:63];
    integer j; // Declare it here, outside the block

    assign instr = (addr < 256) ? Rom[addr[7:2]] : 32'h00000013; 
...
module Register_File(
    input clk, rst, RegWrite, write_aes,
    input [4:0] Rs1, Rs2, Rd, 
    input [31:0] Write_data,
    input [127:0] data_aes,
    output [31:0] read_data1, read_data2
);
    reg [31:0] Registers [31:0];
    integer k;
    
    // This ensures values exist even BEFORE a reset pulse
    initial begin
        for (k = 0; k < 32; k = k + 1) Registers[k] = 32'b0;
        Registers[10] <= 32'h0011_2233;
        Registers[11] <= 32'h4455_6677;
        Registers[12] <= 32'h8899_aabb;
        Registers[13] <= 32'hccdd_eeff;
    end
    ...
    always @(posedge clk or posedge rst) begin
        ...
        else if (write_aes) begin
            Registers[0] <= 32'h0011_2233;
            Registers[1] <= 32'h4455_6677;
            Registers[2] <= 32'h8899_aabb;
            Registers[3] <= 32'hccdd_eeff;
            Registers[10] <= data_aes[127:96];
            Registers[11] <= data_aes[95:64];
            Registers[12] <= data_aes[63:32];
            Registers[13] <= data_aes[31:0];
        end 
...
module RISCV_AES_Top(input clk, rst);
    wire [31:0] pc_curr, instr;
    wire [31:0] rd1, rd2;
    wire [127:0] aes_out;
    wire aes_busy, is_aes;
    
    wire [31:0] pc_next = pc_curr + 4;
    
    // --- FIX 1: Initialize busy_q to 0 and handle reset ---
    reg busy_q = 0; 
    always @(posedge clk or posedge rst) begin
        if (rst) busy_q <= 0;
        else busy_q <= aes_busy;
    end

    // --- FIX 2: Strict transition detection (1 to 0 only) ---
    // This prevents the registers from being overwritten at Time 0
    wire aes_done_pulse = (busy_q == 1'b1 && aes_busy == 1'b0); 

    wire [127:0] aes_key = 128'h000102030405060708090a0b0c0d0e0f;
    assign is_aes = (instr == 32'h04250533);

    // -------- AES start pulse generator --------
    reg is_aes_d;

    wire aes_start = is_aes & ~aes_busy;



    wire [127:0] reg_plaintext = {RF.Registers[10], RF.Registers[11], RF.Registers[12], RF.Registers[13]};

    program_counter PC (
        .clk(clk), .rst(rst), .stall(aes_busy), .pc_in(pc_next), .pc_out(pc_curr)
    );

    Instruction_Memory IM (
        .addr(pc_curr), .instr(instr)
    );

    Register_File RF (
        .clk(clk), .rst(rst),
        .RegWrite(!aes_busy && !is_aes),
        .write_aes(aes_done_pulse),
        .Rs1(instr[19:15]),
        .Rs2(instr[24:20]),
        .Rd(instr[11:7]),
        .Write_data(32'b0),
